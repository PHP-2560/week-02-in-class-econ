...
```{r}
setwd("C:/Users/gabriel/Dropbox (Brown)/III_year/PHP2560/week-02-in-class-econ")
```
...
#the command changes the names of the first three columns and rename progressively the remaining ones
```
...
rain.df <- mutate(rain.df, daily = rowSums(select(rain.df, "0":"23")))
hist(rain.df$daily)
```
...
hist(rain.df$daily)
#the new graphs shows a more reasonable distribution of daily rainfalls as it does not contain negative values. 
