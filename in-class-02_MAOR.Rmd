...
```{r}
setwd("C:/Users/yaela/Dropbox (Brown)/Brown/R/in-class-work-2018-maormilgrom/week-02-in-class-econ")  
...
```{r}
filter(flights, month == 9 & day == 19)  # all flights on August 1st
...
```{r}
flights %>%
arrange(year, month, day)  %>% # arrange by year, month, day
...
```{r}
flights %>%
  summarize(avg_delay=mean(arr_delay+dep_delay,na.rm=TRUE)) %>%
  select(avg_delay)
```
...
flights %>%
  group_by(tailnum) %>%
  summarize(count=n(),dist=mean(distance,na.rm=T),delay=mean(arr_delay+dep_delay,na.rm=TRUE)) %>%
...
It is difficult to follow. UGLY!
...
new_code=flights %>%
  ...
  summarize( arr = mean(arr_delay, na.rm = TRUE), dep = mean(dep_delay, na.rm = TRUE)) %>%
  filter(arr > 30 | dep > 30)

```
d. Does your rewritten command give the same results?
```{r}
all_equal(original_code,new_code)
```
