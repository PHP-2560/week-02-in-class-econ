...
```{r}
setwd("~/Dropbox (Brown)/brown/cursos/3ro/r/in-class-work-2018-pablogarriga/week-02-in-class-econ")
library(dplyr)
rain.df=tbl_df(read.table("rnf6080.dat"))
```
...
tail(rain.df)
```
The last 24 columns represent each of the day's hours.
...
```{r}
rain.df <- mutate(rain.df, daily = rowSums(select(rain.df, '1':'23')))
hist(rain.df$daily)
```
